...
the member name. */
SELECT DISTINCT f.name, CONCAT(surname, ', ', firstname)  AS 'Member'
FROM `Members` m
...
SELECT f.name AS 'Facility Name', CONCAT(m.surname, ', ', m.firstname) AS 'Member/Guest',
...
SELECT sub.name, CONCAT( sub.surname, ', ', sub.firstname ) AS 'Member/Guest', sub.cost AS 'Cost'
FROM (
	SELECT f.name, m.surname, m.firstname, b.starttime, CASE 
	WHEN m.memid =0 THEN f.guestcost * b.slots
	ELSE ( f.membercost * b.slots ) END AS cost
	FROM `Members` m
	JOIN `Bookings` b ON m.memid = b.memid
	JOIN `Facilities` f ON b.facid = f.facid) sub
WHERE sub.starttime LIKE '2012-09-14%' AND Cost > 30
ORDER BY cost DESC
